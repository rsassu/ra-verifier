...
# known digests
DIGEST_EMPTY_FILE = 'da39a3ee5e6b4b0d3255bfef95601890afd80709'
DIGEST_NEWLINE_FILE = 'adc83b19e793491b1c6ea0fd8b46cd9f32e592fc'
...
class IMATemplateDesc(object):
    def get_event_digest_sha1(self, data):
        if EVENTDIGEST_NG_FIELD in self.fields:
            d_split = self.get_field_data(EVENTDIGEST_NG_FIELD, data).split(':')
            if d_split[0] != 'sha1':
...
class IMARecord(object):
    ...
    def parse_entry(self):
        ...
        self.entry['template_data'] = d[3:]
        self.entry['event_digest'] = template_desc.get_event_digest_sha1(d[3:])
        self.entry['event_name'] = template_desc.get_event_name(d[3:])

    def get_data(self, field_name):
        return self.entry['template_desc'].get_field_data(field_name,
    ...
    def execute_digests_query(cls, conn, distro):
        ...
        for digest_obj in list(cls.digests_dict.values()):
            ...
            if digest_string not in query_result:
                log_info('Digest %s (%s) not found' % (digest_string,
            ...
            data = [query_result[digest_string][super_column]
                    ...
                    if super_column.startswith(distro)]

            if len(data) == 0:
                log_info('Digest %s (%s) not found for distro %s' %
                         (digest_string,
                         digest_obj.ima_records[0].entry['event_name'], distro))
                digest_obj.event_type = ''
                continue
